...
{
    public class TestFixtureEolTestSequenceModel 
    {
        #region constructor
        public TestFixtureEolTestSequenceModel()
        {
        ...
        private void DeserializeAndSetProperties(string filepath)
        {
            ...
            {
                frmTestFixture.Instance.WriteToLog("TestFixtureEolTesterModel ERROR: " + e.Message, ApplicationConstants.TraceLogType.Error);
            }
        ...
        private bool _modelNumber = false;
        [JsonIgnore]
        ...
        [JsonProperty("_bandwidth")]
        private bool _bandwidth = false;
        [JsonIgnore]
        ...
        {
            if ("_modelNumber" == file.Name)
            { ModelNumber = file.Value; }
            else if ("_serialNumber" == file.Name)
            { SerialNumber = file.Value; }
            else if ("_powerOn" == file.Name)
            { PowerOn = file.Value; }
            else if ("_pairing" == file.Name)
            { Pairing = file.Value; }
            ...
            else if ("_bandwidth" == file.Name)
            { Bandwidth = file.Value; }
